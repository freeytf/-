...
class RedisClient
{
    ...
    //连接属性数组
    protected $option=RedisConfig::REDIS_CONFIG;

    private function __construct($db=null,array $option=[])
    {

        if (!extension_loaded('redis')) {   //判断是否有扩展(如果你的apache没reids扩展就会抛出这个异常)
            throw new Exception('not support: redis',404);
        }

        $this->option           =    array_merge($this->option,$option);
        $this->redis            =    new \Redis();
        $func = $this->option['persistent'] ? 'pconnect' : 'connect';
        $this->redis->$func($this->option['host'],$this->option['port'], $this->option['timeout']);
        $this->redis->auth($this->option['auth']);
        $this->redis->select($db??$this->option['db_id']);
        $this->option['expireTime']    =    time() + $this->option['timeout'];
    }
    ...
    public static function getInstance($db=0,$config=[])
    {
        ...
        if(! (self::$_instance instanceof self)) static::$_instance= new self($db,$config);
    ...
    public function sAdd($key,$value)
    {
        if(is_array($value)){
            foreach($value as $row){
                $result=$this->redis->sAdd($key,$row);
            }
        }else{
